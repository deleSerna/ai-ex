...
public class RoutingWorkflow {
    ...
    public String route(String input, Map<String, String> routes) {
        ...
        RoutingResponse route = determineRoute(input, routes.keySet());

        // Get the selected prompt from the routes map
        System.out.println("Routekey is="+route.selection());
        String clientEndpoint = routes.get(route.selection());

        if (clientEndpoint == null) {
            throw new IllegalArgumentException("Selected route is '" + route.selection() + "' not found in routes map");
        }

        // Process the input with the selected prompt
        //return chatClient.prompt(selectedPrompt + "\nInput: " + input).call().content();
        UriComponentsBuilder builder = UriComponentsBuilder
                .fromUriString("http://localhost:"+clientEndpoint)
                .queryParam("q", route.extract());
    ...
    private RoutingResponse determineRoute(String input, Iterable<String> availableRoutes) {
        ...
        String selectorPrompt = String.format("""
                ...
                \\{
                    ...
                    "extracted":"Extract the name from the input"
                \\}
        ...
        System.out.println(String.format("Routing Analysis:%s\nSelected route: #%s\n Extracted value: #%s#",
                routingResponse.reasoning(), routingResponse.selection(), (routingResponse.extract() == null? "paul": routingResponse.extract())));
